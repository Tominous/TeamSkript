...
public class TeamSkript extends JavaPlugin {
	
	private static TeamSkript instance;
	...
	public static TS3Api ts3api;
	  ...
	  {
		  Bukkit.getLogger().info(ChatColor.translateAlternateColorCodes('&', "&bTeamSkript Disabled!"));
	  }
	  ...
	  {
		  instance = this;
		  TeamSkript.getInstance().registerAll();
		  Bukkit.getLogger().info(ChatColor.translateAlternateColorCodes('&', "&aTeamSkript Enabled!"));
	  }
		   ...
		   Classes.registerClass(new ClassInfo<Client>(Client.class, "client").user("client").name("client").parser(new Parser<Client>() {
			...
			public Client parse(String arg0, ParseContext arg1) {
				...
				if(arg0.startsWith("client ")) {
					return TeamSkript.ts3api.getClientByNameExact(arg0.replaceFirst("client ", ""), true);
				}
				else {
					return TeamSkript.ts3api.getClientByNameExact(arg0, true);
				}
			}
